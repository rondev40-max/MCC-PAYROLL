...
class AdminPersonnelTimesheetController extends Controller
{
    public function index(Request $request)
    {
        ...
        $timesheets = AdminPersonnelTimesheet::where('year', $year)
                        ->where('month', $month)
                        ->where('period', $period)
                        ->get();
        ...
        if ($timesheets->isEmpty()) {
            ...
            $uniqueEmployeeNames = AdminPersonnelTimesheet::whereNotNull('employee_name')
                                                 ->where('employee_name', '!=', '')
                                                 ->distinct()
                                                 ->pluck('employee_name');
            ...
            if ($uniqueEmployeeNames->isNotEmpty()) {
                foreach ($uniqueEmployeeNames as $name) {
                    ...
                    $latestRecordForEmployee = AdminPersonnelTimesheet::where('employee_name', $name)
                        ->orderBy('year', 'desc')
                        ->orderBy('month', 'desc')
                    ...
                    if ($latestRecordForEmployee) {
                        ...
                        AdminPersonnelTimesheet::updateOrCreate(
                            [
                                'employee_name' => $name,
                                'month' => $month,
                                'year' => $year,
            ...
            $timesheets = AdminPersonnelTimesheet::where('year', (string)$year) // Tiyaking string ang year para sa tamang paghahanap
                ->where('month', $month)
                ->where('period', $period)
                ->get();
        }
        ...
        return view('admin_personnel.index', compact('timesheets', 'days', 'month', 'year', 'startDay', 'endDay', 'period', 'holidays'));
    }


    public function create()
    {
        return view('admin_personnel.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        AdminPersonnelTimesheet::create($validatedData);
        return redirect()->route('admin-personnel.index')->with('success', 'Admin Personnel timesheet added!');
    }

    // Walang pagbabago sa edit(), update(), at destroy()
    public function edit($id)
    {
        $timesheet = AdminPersonnelTimesheet::findOrFail($id);
        return view('admin_personnel.edit', compact('timesheet'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('admin-personnel.index')->with('success', 'Admin Personnel timesheet updated!');
    }
    ...
    public function destroy($id)
    {
        $timesheet = AdminPersonnelTimesheet::findOrFail($id);
        $timesheet->delete();
        return redirect()->route('admin-personnel.index')->with('success', 'Admin Personnel timesheet deleted!');
    }
    ...
    public function printAll(Request $request)
    {
        ...
        return view('admin_personnel.print', compact('timesheets', 'days', 'holidays', 'month', 'year', 'period'));
    }
}