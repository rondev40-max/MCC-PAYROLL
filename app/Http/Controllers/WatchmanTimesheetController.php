...
class WatchmanTimesheetController extends Controller
{
    public function index(Request $request)
    {
        ...
        $timesheets = WatchmanTimesheet::where('year', $year)
                        ->where('month', $month)
                        ->where('period', $period)
                        ->get();
        ...
        if ($timesheets->isEmpty()) {
            ...
            $uniqueEmployeeNames = WatchmanTimesheet::whereNotNull('employee_name')
                                                 ->where('employee_name', '!=', '')
                                                 ->distinct()
                                                 ->pluck('employee_name');
            ...
            if ($uniqueEmployeeNames->isNotEmpty()) {
                foreach ($uniqueEmployeeNames as $name) {
                    ...
                    $latestRecordForEmployee = WatchmanTimesheet::where('employee_name', $name)
                        ->orderBy('year', 'desc')
                        ->orderBy('month', 'desc')
                    ...
                    if ($latestRecordForEmployee) {
                        ...
                        WatchmanTimesheet::updateOrCreate(
                            [
                                'employee_name' => $name,
                                'month' => $month,
                                'year' => $year,
            ...
            $timesheets = WatchmanTimesheet::where('year', (string)$year) // Tiyaking string ang year para sa tamang paghahanap
                ->where('month', $month)
                ->where('period', $period)
                ->get();
        }
        ...
        return view('watchman.index', compact('timesheets', 'days', 'month', 'year', 'startDay', 'endDay', 'period', 'holidays'));
    }
    ...
    public function store(Request $request)
    {
        ...
        WatchmanTimesheet::create($validatedData);
        return redirect()->route('watchman.index')->with('success', 'Watchman timesheet added!');
    }

    // Walang pagbabago sa edit(), update(), at destroy()
    public function edit($id)
    {
        $timesheet = WatchmanTimesheet::findOrFail($id);
        return view('watchman.edit', compact('timesheet'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('watchman.index')->with('success', 'Watchman timesheet updated!');
    }
    ...
    public function destroy($id)
    {
        $timesheet = WatchmanTimesheet::findOrFail($id);
        $timesheet->delete();
        return redirect()->route('watchman.index')->with('success', 'Watchman timesheet deleted!');
    }
    ...
    public function printAll(Request $request)
    {
        ...
        return view('watchman.print', compact('timesheets', 'days', 'holidays', 'month', 'year', 'period'));
    }
}