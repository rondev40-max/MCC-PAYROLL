...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .night-mode .sidebar .dropdown-item {
      color: var(--text);
    }

    .night-mode .sidebar .dropdown-item:hover {
        ...
        <div class="dropdown">
          ...
          <ul class="dropdown-menu">
            ...
            <li><a class="dropdown-item d-flex align-items-center justify-content-between gap-2" href="{{ route('admin.attendance-payroll.index') }}">
              ...
              @php $pendingReviews = \App\Models\AttendanceSession::whereIn('status', ['open','needs_review'])->count(); @endphp
              @if($pendingReviews > 0)
                <span class="badge bg-warning text-dark" title="{{ $pendingReviews }} session(s) need review" style="font-size:.65rem;">{{ $pendingReviews }}</span>
              @endif
            </a></li>
      ...
      <header class="topbar">
        ...
        <div class="topbar-actions">
          ...
          <button class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center"
            id="nightModeToggle" onclick="toggleNightMode()"
            style="width: 34px; height: 34px; border-radius: var(--r-sm);">
